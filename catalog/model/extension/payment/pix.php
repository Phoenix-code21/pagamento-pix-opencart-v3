<?php
class ModelExtensionPaymentPIX extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('payment_pix_total') > 0 && $this->config->get('payment_pix_total') > $total) {
			$status = false;
		} elseif (!$this->cart->hasShipping()) {
			$status = false;
		} elseif (!$this->config->get('payment_pix_geo_zone_id')) {
			$status = true;
